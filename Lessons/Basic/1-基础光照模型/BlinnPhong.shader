//BlinnPhong就是Phong的变体，只是改变了其中一点计算方法
Shader "MineselfShader/Basic/1-BasicLightingModel/BlinnPhong"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float3 hDir = normalize(lDir + vDir);//BlinnPhong需要的是hDir，由于两者相加后长度不为1，所以需要归一化
                ...
                //===BlinnPhong高光反射项===
                //与Phong略有不同，为hDir·nDir，pow()可以"调整高光范围"
                float specular = pow(saturate(dot(hDir, nDir)), _Gloss);
